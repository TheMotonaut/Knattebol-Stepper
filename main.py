...
from TMC5240_bob import TMC5240_bob
import logging
...
def main():
    ...
    logging.basicConfig(filename='bajs.log', level=logging.DEBUG)

    manager = ConnectionManager(arg_list=["--interface", "spi_ic"])

    print(manager)

    connection = manager.connect()

    board = TMC5240_bob(connection)
    motor = board.motors[0]
    mc = board.ics[0]

    print("Preparing parameters")
    # preparing drive settings
    board.write_register_field(mc.FIELD.IRUN, 17)
    board.write_register_field(mc.FIELD.IHOLD, 6)
    board.write_register_field(mc.FIELD.MRES, 5)  # 256 micro-steps
    print("Max_Velocity:", board.read_register(mc.REG.VMAX))
    print("Max_Acceleration:", board.read_register(mc.REG.AMAX))

    time.sleep(1.0)

    # clear position counter
    motor.actual_position = 0
    board.write_register(mc.REG.AMAX, 500)

    # set up StallGuard2
    print("Configuring StallGuard2 parameters...")
    stallguard2_init(mc, motor, board, init_velocity=5000)
